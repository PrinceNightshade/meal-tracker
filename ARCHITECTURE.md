...
1. **User opens modal** → Recents rendered, search empty
2. **User clicks a food** → Serving picker modal pops up
3. **User confirms serving** → Food added to meal, picker closes
4. **Original modal stays open** → Ready for next food
5. **User clicks outside/back** → Modal closes

When a food is just-added, it may now appear in the "recent" section if you re-add it immediately.
...
**Cache key:** `meal-tracker-vN` (e.g., `meal-tracker-v52`)
...
**Update mechanism:**
- Every code change bumps `const CACHE = 'meal-tracker-vN'` in `sw.js` (increment N)

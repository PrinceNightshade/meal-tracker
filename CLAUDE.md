...
- [~] **Phase 3 — auto-sync via Apple Health (code done Sep 2026; awaiting Eric's deploy).** `functions/syncWellness` (gen2 Cloud Function): POST + `X-Sync-Secret` shared secret, merge-writes `users/{uid}/wellness/{date}` via Admin SDK (matches the shape `pullFromCloud` reads). `firebase.json`/`.firebaserc` added. App surfaces the Firebase uid ("Sync ID") in Goals → Wellness Data for setup. **Remaining (Eric, manual):** enable Blaze, `firebase deploy --only functions`, set `SYNC_SECRET`, build the nightly iOS Shortcut — all steps in [functions/README.md](functions/README.md). Chosen host: Cloud Function (over Cloudflare Worker) — one ecosystem, ~$0 in free tier.
- [x] **Layout rethink → opportunity-first home (Sep 2026).** Daily view restructured: opportunity statement owns the top (`renderOpportunityCard` standalone), then the smart quick-log card, a compact intake readout (`renderCompactIntake`, carries the #btn-prev/#btn-next day nav via the delegated `bindNav` handler), a steps/sleep/weight vitals strip (`renderVitalsStrip`), then the action row + meals. Removed the dead ring carousel (`renderDailySummaryCarousel`/`renderDailySummaryRings`/`renderMacroCard`). **Time-aware home** parked for later. Persistent-FAB idea dropped in favor of keeping the proven action row + the smart card for one-tap logging.
- [x] **Smart logging suggestions (Sep 2026).** `js/suggest.js`: `currentMealSlot()` (time-of-day) + `getSmartSuggestions(mealType)` — detects the consistent combo for a slot ("your usual lunch", surfaced ≥3 occurrences AND ≥40% of logged days) and ranks frequent single items by count+recency. Home shows a one-tap re-log card for the current slot (today only); still requires the confirm tap so awareness stays (see [[project_value_prop]]). TODO: also lead the add-food sheet with these (the `SmartLog` design board) — not yet wired into the modal.
